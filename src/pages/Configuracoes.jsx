...
export default function Configuracoes({ activeTeam }) {
  ...
  useEffect(() => { load(); loadCustomSessions() }, [activeTeam])
          ...
          <div className="card">
            ...
            {customSessions.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '20px 0', color: 'var(--muted)', fontSize: 12 }}>
                Nenhuma sessão personalizada ainda
              </div>
            ) : (
                ...
                {customSessions.map(s => (
                  <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px',
                    ...
                    <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 4,
                      ...
                      color: s.type === 'treinamento' ? 'var(--auvo)' : 'var(--green)', fontWeight: 600, flexShrink: 0 }}>
                      Dia {s.day}
                    </span>
                    <span style={{ fontSize: 12, flex: 1 }}>{s.title}</span>
                    <span style={{ fontSize: 9, color: 'var(--muted)', flexShrink: 0 }}>
                      {s.type === 'treinamento' ? '📚 Treino' : '🎯 Simulação'}
                    </span>
