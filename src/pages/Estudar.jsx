    ...
    <div style={{ padding: '24px', maxWidth: 860, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--muted2)' }}>
          Conteúdos disponíveis para o seu onboarding · {analyst.team === 'atendimento' ? '🎧 Atendimento' : '💼 Vendas'}
        </div>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }}>
          {filtered.map(item => {
            ...
            return (
              ...
              >
                {/* Thumb */}
                <div style={{ height: 100, background: 'var(--surface2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30, position: 'relative' }}>
                  {ytId ? (
                ...
                <div style={{ padding: '12px 14px' }}>
                  <div style={{ fontSize: 12, fontWeight: 600, lineHeight: 1.3, marginBottom: 5, color: 'var(--text)' }}>{item.title}</div>
                  {item.description && (
                    <div style={{ fontSize: 10, color: 'var(--muted2)', lineHeight: 1.5, marginBottom: 8 }}>{item.description}</div>
                  )}

                  {/* Session chips */}
                  {item.session_keys?.length > 0 && (
                    <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap', marginBottom: 8 }}>
                      <span style={{ fontSize: 9, color: 'var(--muted)' }}>Vinculado às sessões:</span>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--auvo)', fontWeight: 500 }}>
                    {item.type === 'youtube' && '▶ Assistir vídeo'}
                    {item.type === 'pdf' && '📄 Abrir PDF'}
                    {item.type === 'notebooklm' && '🤖 Abrir no NotebookLM'}
                    {item.type === 'texto' && '📝 Ler conteúdo'}
                    {item.type === 'playbook' && '📘 Abrir playbook'}
                    {item.type === 'link_util' && '🔗 Acessar link'}
                  </div>
                </div>
              </div>
            )
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setPreview(null)}>
          <div className="modal" style={{ width: 700 }}>
            ...
            <div style={{ padding: 20 }}>
              ...
              {preview.description && <div style={{ marginTop: 14, fontSize: 12, color: 'var(--muted2)', lineHeight: 1.6 }}>{preview.description}</div>}
            </div>
          </div>
        </div>
      )}
