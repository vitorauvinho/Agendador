...
export default function MinhaGamificacao() {
  ...
  function fmtReason(r) {
    const map = {
      ...
      exercicio_enviado: 'Exercício enviado',
      video_enviado: 'Vídeo enviado',
      csat_respondido: 'CSAT respondido',
        ...
        <div style={{ background: 'linear-gradient(135deg, var(--auvo), var(--auvo-dark))', borderRadius: 16, padding: 22, marginBottom: 20, color: '#fff' }}>
          <div className="flex items-center gap-4">
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.18)', borderRadius: 4, height: 6, overflow: 'hidden', marginTop: 8, width: 240 }}>
            ...
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
                {BADGES.map(b => {
                  ...
                  return (
                    <div key={b.id} title={b.desc} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '10px 4px', borderRadius: 9, border: `1px solid ${earned ? 'var(--auvo-border)' : 'var(--border)'}`, background: earned ? 'var(--auvo-dim)' : 'transparent', opacity: earned ? 1 : 0.28, filter: earned ? 'none' : 'grayscale(1)' }}>
          ...
          <div className="card" style={{ alignSelf: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {ranking.map((r, i) => {
                ...
                return (
                  <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 10px', borderRadius: 8, border: `1px solid ${isMe ? 'var(--auvo-border)' : 'var(--border)'}`, background: isMe ? 'var(--auvo-dim)' : 'var(--surface2)' }}>
                    <span style={{ fontSize: 14, width: 20, flexShrink: 0, textAlign: 'center' }}>{i < 3 ? medals[i] : i + 1}</span>
