...
export default function Onboarding({ activeTeam }) {
  ...
  async function loadAnalysts(preserveSelected = false) {
    ...
    const { data: analystData, error } = await supabase
      .from('analysts').select('*').eq('team', activeTeam)
      .not('email', 'like', 'rq\_%').order('created_at', { ascending: false })
  ...
  async function deleteEx(id) {
    ...
    setConfirmDeleteEx(null); loadExercicios()
  }
  ...
  async function loadSessions(analystId) {
    const { data } = await supabase
      .from('sessions').select('*, exercises(*), video_submissions(*), session_ratings(*)')
      .eq('analyst_id', analystId).order('day_number')
    setSessions(data || [])
  }
  ...
  async function addAnalyst() {
    ...
    if (form.turmaMode) {
      ...
      for (const a of allTurmaAnalysts) {
        const { data: newA, error } = await supabase.from('analysts')
          .insert({ name: a.name, email: a.email, team: activeTeam, start_date: form.startDate, status: 'ativo', role: roles.find(r => r.id === form.roleId)?.name || null })
          .select().single()
        ...
        await supabase.from('notifications').insert({
          team: activeTeam, type: 'session_completed', analyst_id: newA.id,
      ...
      setShowAdd(false)
      setForm({ name: '', email: '', startDate: '', mode: 'all', picked: [], turmaMode: false, turmaIds: [], turmaAnalysts: [], roleId: '' })
      setSaving(false)
      await loadAnalysts()
    ...
    const { data: analyst, error } = await supabase.from('analysts')
      .insert({
        name: form.name, email: form.email, team: activeTeam,
        start_date: form.startDate, access_token: token, status: 'ativo',
        role: roles.find(r => r.id === form.roleId)?.name || null
      })
      .select().single()
  ...
  async function saveExercise(sessionId) {
    ...
    setSavingExercise(false); setShowExerciseForm(null); setExerciseFormUrl('')
  ...
  async function saveNote(sessionId) {
    ...
    setShowNote(null); loadSessions(selectedId)
  }
    ...
    <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '22px 24px' }}>
      ...
      <div className="flex items-center justify-between" style={{ marginBottom: 18 }}>
        ...
        <div className="flex gap-2" style={{ alignItems: 'center' }}>
          ...
          <button className="btn btn-primary" onClick={() => { setForm({ name:'', email:'', startDate:'', mode:'all', picked: allSessions.map(s=>s.id), turmaMode: false, turmaIds: [], turmaAnalysts: [], roleId: '' }); setShowAdd(true) }}>
            + Novo analista
          </button>
        </div>
      </div>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 12, overflowY: 'auto' }}>
                {trilhas.map(t => (
                  <div key={t.id} onClick={() => setSelectedTrilha(t)}
                    style={{ padding: '9px 11px', borderRadius: 8, cursor: 'pointer', marginBottom: 5, border: `1px solid ${selectedTrilha?.id===t.id?'var(--auvo-border)':'var(--border)'}`, background: selectedTrilha?.id===t.id?'var(--auvo-dim)':'var(--surface2)' }}>
                  ...
                  return (
                    <>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                        <thead>
                          <tr>{['Analista','Progresso','Vídeos','Último acesso'].map(h=>(
                        ...
                        <tbody>
                          {analystRows.map(a => (
                            <tr key={a.id}>
                              ...
                              <td style={{ padding: '10px', borderBottom: '1px solid var(--border)', color: 'var(--muted)' }}>{a.lastWatch ? new Date(a.lastWatch.watched_at).toLocaleDateString('pt-BR') : '—'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 12, overflowY: 'auto' }}>
                {analysts.filter(a=>a.status==='ativo').map(a => (
                  <div key={a.id} onClick={() => setSelectedAnalystTrilha(a)}
                    style={{ padding: '9px 11px', borderRadius: 8, cursor: 'pointer', marginBottom: 5, border: `1px solid ${selectedAnalystTrilha?.id===a.id?'var(--auvo-border)':'var(--border)'}`, background: selectedAnalystTrilha?.id===a.id?'var(--auvo-dim)':'var(--surface2)' }}>
                    <div style={{ fontSize: 12, fontWeight: 600 }}>{a.name}</div>
                    <div style={{ fontSize: 10, color: 'var(--muted2)', marginTop: 2 }}>
                      {trilhas.filter(t => { const items = t.video_trail_items || []; return items.length > 0 && items.every(i => trilhaProgress.find(p=>p.analyst_id===a.id&&p.item_id===i.id&&p.watched)) }).length}/{trilhas.length} trilhas concluídas
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 14, flex: 1, overflow: 'hidden' }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 14, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {loading ? (
              <div className="flex items-center justify-between" style={{ padding: '40px 0', justifyContent: 'center' }}><div className="spinner" /></div>
              ...
              return (
                <div key={a.id} onClick={() => setSelectedId(isSel ? null : a.id)}
                  style={{ padding: '10px 12px', borderRadius: 9, cursor: 'pointer', border: `1px solid ${isSel ? 'var(--auvo-border)' : 'var(--border)'}`, background: isSel ? 'var(--auvo-dim)' : 'var(--surface2)', opacity: isExited ? 0.55 : 1, transition: 'all 0.15s' }}>
                  <div className="flex items-center gap-2" style={{ marginBottom: 8 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--muted2)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {a.role ? <span style={{ color: 'var(--auvo)' }}>{a.role} · </span> : ''}{a.email}
                      </div>
                    </div>
                    <span className={`status-pill status-${a.status}`}>{a.status}</span>
                    <button onClick={e => { e.stopPropagation(); setConfirmDeleteAnalyst(a.id) }}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--muted)', padding: '2px 4px', borderRadius: 4, flexShrink: 0 }}>🗑️</button>
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-3" style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--muted2)', marginTop: 2 }}>
                      {selected.role && <span style={{ color: 'var(--auvo)', fontWeight: 600 }}>{selected.role} · </span>}
                      {selected.email} · Início em {fmtDateLong(selected.start_date)}
                    </div>
                  </div>
                            ...
                            {daySessions.map(session => {
                              ...
                              return (
                                <div key={session.id} style={{ marginBottom: 4 }}>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', border: `1px solid ${session.completed ? 'rgba(16,185,129,0.15)' : 'var(--border)'}`, borderRadius: 8, cursor: 'pointer', transition: 'all 0.1s', background: session.completed ? 'rgba(16,185,129,0.04)' : 'var(--surface2)', fontSize: 11, opacity: (!canComplete && !session.completed) ? 0.6 : 1 }}
                                    ...
                                    <span style={{ flex: 1, textDecoration: session.completed ? 'line-through' : 'none', color: session.completed ? 'var(--muted)' : 'var(--text)' }}>{session.title}</span>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setConfirmDeleteAnalyst(null)}>
          <div className="modal" style={{ width: 400 }}>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-danger" onClick={() => deleteAnalyst(confirmDeleteAnalyst)}>Sim, excluir permanentemente</button>
            </div>
          </div>
        </div>
      )}
              ...
              {roles.length > 0 && (
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      setForm(x => ({
                        ...x,
                        roleId,
                        mode: role?.session_ids?.length > 0 ? 'custom' : x.mode,
                        picked: role?.session_ids?.length > 0
                          ? allSessions.filter(s => role.session_ids.includes(String(s.id))).map(s => s.id)
                          : x.picked,
                      }))
                    }}
                  ...
                  >
                    ...
                    {roles.map(r => (
                      <option key={r.id} value={r.id}>{r.name} — {r.session_ids?.length || 0} sessões</option>
                    ))}
                  </select>
                  {form.roleId && (
                    <div style={{ fontSize: 10, color: 'var(--green)', marginTop: 4 }}>
                      ✓ Sessões pré-selecionadas — você ainda pode editar abaixo
                    </div>
                  )}
                </div>
              )}
                ...
                {form.turmaMode && (
                  <div style={{ background: 'var(--surface2)', borderRadius: 8, padding: 12, border: '1px solid var(--border)' }}>
                    <div style={{ fontSize: 11, color: 'var(--muted2)', marginBottom: 10 }}>
                      O analista acima + os que você adicionar abaixo entrarão nas mesmas sessões:
                    </div>
                    {form.turmaAnalysts.map((a, idx) => (
                      <div key={idx} style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center' }}>
                        <input value={a.name} onChange={e => setForm(x => ({ ...x, turmaAnalysts: x.turmaAnalysts.map((t,i) => i===idx ? {...t, name: e.target.value} : t) }))} placeholder="Nome" style={{ flex: 1, fontSize: 11 }} />
                        <input value={a.email} onChange={e => setForm(x => ({ ...x, turmaAnalysts: x.turmaAnalysts.map((t,i) => i===idx ? {...t, email: e.target.value} : t) }))} placeholder="Email" type="email" style={{ flex: 1, fontSize: 11 }} />
                        <button onClick={() => setForm(x => ({ ...x, turmaAnalysts: x.turmaAnalysts.filter((_,i) => i!==idx) }))} style={{ background: 'none', border: 'none', color: 'var(--red)', cursor: 'pointer', fontSize: 16, padding: '0 4px', flexShrink: 0 }}>×</button>
                      </div>
                    ))}
                    ...
                    {form.turmaAnalysts.length > 0 && (
                      <div style={{ marginTop: 8, fontSize: 10, color: 'var(--green)' }}>✓ {1 + form.turmaAnalysts.length} analistas serão cadastrados juntos</div>
                    )}
                  </div>
                )}
              </div>
                ...
                <div className="flex gap-2">
                  {[['all','📅 Agendar tudo'],['custom','✏️ Escolher sessões']].map(([k,l]) => (
                    <button key={k} onClick={() => setForm(x => ({ ...x, mode: k }))}
                      style={{ flex: 1, padding: '9px', borderRadius: 7, cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, border: `1px solid ${form.mode===k?'var(--auvo-border)':'var(--border)'}`, background: form.mode===k?'var(--auvo-dim)':'transparent', color: form.mode===k?'var(--auvo)':'var(--muted2)', fontWeight: form.mode===k?600:400 }}>
                      {l}
                    </button>
                  ))}
                </div>
